...
- なぜコードに初期化が必要か？
1. 変数に「ゴミ」が入っているため
プログラムの変数に初期値を設定しない場合、その変数が確保されたメモリ領域には、以前に使われたデータがそのまま残っていることがある。これを「ゴミ」や「不定値」と呼ぶ。

2. 計算の基準点を設定するため
合計を計算したり、回数を数えたりする際、初期値は計算の「出発点」や「基準点」となる。

- 合計（sum）: 0から足し始める
- カウント（count）: 0から数え始める
- 掛け算（product）: 1からかけ始める

例えば、strikesの数を数える場合、strikesを0で初期化することで、「まだストライクが一つもない状態」という基準を明確に設定できます。ここから1ずつ増やしていくことで、正確な数を数えることができるようになる。

## 2025-08-13
rubyの「.abs」 は「絶対値 (absolute value)」を計算するメソッドのこと。
- .abs は "absolute"（アブソリュート） の略で、「絶対値」を意味する。
- どんな数値に対しても、符号（プラスかマイナスか）を取り除き、常に正の数（またはゼロ）を返す役割がある。
- 例えば、5 の絶対値は 5 ですし、-5 の絶対値も 5 。
「マイナス符号を無視して、数字の大きさだけを知りたいとき」に使う便利なメソッド！
