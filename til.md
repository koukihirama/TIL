...
## 2025-08-01
- TurboとはHotwireの一部で、SPA（シングルページアプリケーション）のような体験を提供するJavaScriptフレームワークのこと。
Turboの特徴は、ページ全体を再読み込みせずに画面の一部だけを更新できる点。
従来のJavaScriptフレームワークのように複雑な構成や設定を必要とせず、Rails標準の機能と連携して動作する。
開発者はRailsの規約に沿ってコードを書くだけで、バックエンドの処理とフロントエンドの両方をシームレスに扱える。